...
class Configure {
  ...
  encrypt(plainText) {
    if (this.validate("working_key") && plainText) {
      ...
      // console.log(working_key,'------------------enc working key')
  ...
  decrypt(encText) {
    if (this.validate("working_key") && encText) {
      const { working_key } = initOptions;
      // console.log(working_key,'------------------dec working key')
      const m = createHash("md5");
      m.update(working_key);
      const key = m.digest();
      const iv =
  ...
  getEncryptedOrder(orderParams) {
    if (this.validate("merchant_id") && orderParams) {
      // console.log(this.validate('merchant_id'),'--------------validate merchant id')
...
const CCAvenue = new Configure({
  working_key: "B9ED6EB7296BCFF5177418D8454AE2AB", // Your Working Key from CCAvenue
  merchant_id: "3278766", // Your Merchant ID from CCAvenue
});
